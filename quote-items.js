...
(function() {
    'use strict';
        ...
        function renderManageUpgradeSourceSelect(selectedName, selectedCategory) {
            ...
            const options = ['<option value="">Select Existing Item</option>'];
            ...
            groups.forEach(function(group, groupIndex) {
                ...
                group.options.forEach(function(option, optionIndex) {
                    ...
                    html += '<div class="manage-upgrade-option border rounded p-2 mt-2" data-upgrade-option-id="' + manageItemsAttr(option.id) + '">' +
                        ...
                        '<div class="col-md"><label class="form-label" style="font-size:0.75em">Create New Upgrade Name</label><input type="text" class="form-control form-control-sm upgrade-name" value="' + manageItemsAttr(option.name) + '" placeholder="e.g., Post-to-post drink rail" oninput="markPricingDirty(this)"></div>' +
                        '<div class="w-100 manage-upgrade-primary-row-break"></div>' +
                        '<div class="col-md-3"><label class="form-label" style="font-size:0.75em">Unit</label>' + renderManageUnitSelect(option.unitType || baseUnitType || '', 'upgrade-unit-type') + '</div>' +
                        ...
                        '<div class="col-md-11"><label class="form-label" style="font-size:0.75em">Supplier URL</label><input type="url" class="form-control form-control-sm upgrade-supplier-url" value="' + manageItemsAttr(option.supplierUrl) + '" placeholder="https://..." oninput="markPricingDirty(this)"></div>' +
        ...
        function renderManageUpgradeWizardOptionsStep() {
            ...
            return '<div data-upgrade-wizard-step="options">' +
                ...
                '</div>' +
                '<div class="d-flex justify-content-between align-items-center gap-2 flex-wrap mb-2"><div class="small text-muted">Select an existing saved item or create a new upgrade.</div><div class="d-flex gap-1 flex-wrap"><button type="button" class="btn btn-sm btn-outline-secondary" data-upgrade-wizard-action="toggle-note"><i class="fas fa-note-sticky me-1"></i>Add Note</button><button type="button" class="btn btn-sm btn-outline-primary" data-upgrade-wizard-action="add-option"><i class="fas fa-plus me-1"></i>Add Option</button></div></div>' +
                ...
                options.map(function(option, optionIndex) {
                    ...
                    return '<div class="manage-upgrade-wizard-option manage-upgrade-wizard-option-card" data-upgrade-option-id="' + manageItemsAttr(option.id) + '">' +
                        ...
                        '<div class="col-md-auto d-flex align-items-center justify-content-center pt-md-4 manage-upgrade-choice-divider"><span class="small fw-bold text-muted">OR</span></div>' +
                        '<div class="col-md"><label class="form-label small mb-1">Create New Upgrade Name</label><input type="text" class="form-control form-control-sm upgrade-name" value="' + manageItemsAttr(option.name) + '" placeholder="e.g., Post-to-post drink rail"></div>' +
                        '<div class="w-100 manage-upgrade-primary-row-break"></div>' +
                        '<div class="col-md-3"><label class="form-label small mb-1">Unit</label>' + renderManageUnitSelect(option.unitType || state.baseUnitType || '', 'upgrade-unit-type') + '</div>' +
                        ...
                        '<div class="col-md-3"><label class="form-label small mb-1">Cost</label><input type="number" class="form-control form-control-sm upgrade-material-cost" value="' + manageItemsAttr(optionRequiresConsultation ? '0.00' : option.materialCost.toFixed(2)) + '" step="0.01" min="0" ' + (optionRequiresConsultation ? 'disabled' : '') + '></div>' +
                        '<div class="w-100 manage-upgrade-pricing-row-break"></div>' +
