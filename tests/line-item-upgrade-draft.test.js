...
assert(builder.includes('quote-items.js?v=2026091802'), 'new HTML must request the matching item-editor script');
assert(builder.includes('quote-storage.js?v=2026091802'), 'quote storage must use the matching release');
...
ctx.resetLineItemUpgradeDraft({upgradeGroups:[{id:'g',options:[{id:'a',name:'Premium'},{id:'b',name:'Other'}]}]});
ctx.window.toggleLineItemUpgradeForQuote(0,0,false);
assert.equal(ctx.window.getLineItemOfferedUpgradeDraft()[0].options.length,1);
assert.equal(ctx.getLineItemUpgradeDraft()[0].options.length,2,'catalog retained for re-enabling and library saving');
const persisted={quoteUpgradeCatalog:ctx.getLineItemUpgradeDraft(),quoteDisabledUpgrades:ctx.window.getLineItemDisabledUpgrades(),upgradeGroups:ctx.window.getLineItemOfferedUpgradeDraft()};
ctx.resetLineItemUpgradeDraft(JSON.parse(JSON.stringify(persisted)));
assert.equal(ctx.window.getLineItemOfferedUpgradeDraft()[0].options[0].id,'b','disabled survives reopen');
ctx.window.toggleLineItemUpgradeForQuote(0,0,true);
assert.equal(ctx.window.getLineItemOfferedUpgradeDraft()[0].options.length,2);
ctx.window.toggleLineItemUpgradeForQuote(0,0,false);
ctx.window.toggleLineItemUpgradeForQuote(0,1,false);
assert.equal(ctx.window.getLineItemOfferedUpgradeDraft().length,0,'all disabled removes client choices');
assert(summary.innerHTML.includes('Offer on this quote'));
assert.equal(persisted.upgradeGroups[0].options.length,1,'draft toggles do not mutate live quote');
console.log('Per-option quote toggles retain catalog, survive JSON reopen and never modify the library');
