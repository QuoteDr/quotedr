...
assert.equal(ctx.writes,0,'saved categories default to a quote-only override');
assert.equal(ctx.categoryStyles.Saved.icon,'fa-tag');
ctx.applyQuoteCategoryIcon('Saved','fa-hammer',true);
...
assert.equal(ctx.finishes,3,'unknown icon rejected');
