...
export async function quoteDesignState(db, row, viewer, designId) {
  const linkResult = await db.from('quote_design_links').select('*').eq('document_id',row.id).maybeSingle();
  if(linkResult.error) throw linkResult.error;
  const link=linkResult.data;
  if(!link) return null;
  const states=[];
  for(const id of (link.design_ids?.length ? link.design_ids : [link.design_id])){
  const found=await db.from('portal_designs').select('*').eq('id',id).maybeSingle();
  ...
  if(lib.data?.user_id!==row.user_id || lib.data?.portal_id!==row.data?.portal_id)continue;
  const revision=link.updated_at+'|'+design.updated_at+(link.design_ids?.length>1?'|'+design.id:'');
  let receipt=null;
  ...
  states.push({link,design,receipt,revision,locked});
  }
  if(!states.length)return null;
  if(designId)return states.find(s=>s.design.id===designId)||null;
  const current=states.find(s=>s.locked)||states[0];
  return {...current,states,locked:states.some(s=>s.locked)};
}
export function publicDesignReview(state) {
  ...
  return {id:state.design.id,title:state.design.title,version:state.design.version,kind:state.design.kind,revision:state.revision,locked:state.locked,
    ...(state.states?{designs:state.states.map(publicDesignReview)}:{})};
}
